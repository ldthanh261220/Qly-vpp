...
const PlanList = () => {
        ...
        thanhtoanhopdongService
            ...
            .then((response) => {
                const mappedPlans = response.danhsachhopdong.map((contractor) => ({
                    ...
                    time: `${moment(contractor.thoiGianThucHien).format('DD/MM/YYYY')} - ${moment(
                        contractor.thoiGianHoanThanh,
                    ).format('DD/MM/YYYY')}`,
                    type: 'Mua sắm',
    ...
    const filteredPlans = plans.filter((plan) => {
        if (plan.status === 'Bị hủy') return false; // ẩn Bị hủy khỏi danh sách
        if (activeTab === 'all') return true;
        return plan.status === activeTab;
    });

    const counts = {
        all: plans.filter((plan) => plan.status !== 'Bị hủy').length,
        'Đã ký': plans.filter((plan) => plan.status === 'Đã ký').length,
        'Đang thực hiện': plans.filter((plan) => plan.status === 'Đang thực hiện').length,
    };
    ...
    const handleApprove = async (id) => {
        await updatePlanStatus(id, 'Đang thực hiện');
        setPlans(plans.map((plan) => (plan.id === id ? { ...plan, status: 'Đang thực hiện' } : plan)));
        if (selectedPlan && selectedPlan.id === id) {
            setSelectedPlan((prev) => ({ ...prev, status: 'Đang thực hiện' }));
            setActiveTab('Đang thực hiện');
            handleBackToList();
        }
    };
    const handleReject = async (id) => {
        await updatePlanStatus(id, 'Bị hủy');
        setPlans(plans.map((plan) => (plan.id === id ? { ...plan, status: 'Bị hủy' } : plan)));
        handleBackToList();
    };
        ...
        <div className="plan-list">
            {!selectedPlan ? (
                <>
                    ...
                    {filteredPlans.length > 0 && <div className="plan-list__batch-actions"></div>}
                </>
            ) : (
                <div className="plan-list__detail">
                    ...
                    <div className="section bg-gray-100 p-4 rounded mb-4">
                        ...
                        <p>
                            <strong>Mã nhà thầu:</strong> {selectedPlan.contractorCode}
                        </p>
                        <p>
                            <strong>Tên nhà thầu:</strong> {selectedPlan.contractorName}
                        </p>
                        <p>
                            <strong>Tên người đại diện pháp luật:</strong> {selectedPlan.legalRepresentative}
                        </p>
                    </div>
                    <div className="section bg-gray-100 p-4 rounded mb-4">
                        <h3 className="font-semibold">Thông tin chủ đầu tư</h3>
                        <p>
                            <strong>Mã chủ đầu tư:</strong> {selectedPlan.investorCode}
                        </p>
                        <p>
                            <strong>Tên nhà đầu tư:</strong> {selectedPlan.investorName}
                        </p>
                    </div>
                    <div className="section bg-gray-100 p-4 rounded mb-4">
                        <h3 className="font-semibold">Thông tin hợp đồng</h3>
                        <p>
                            <strong>Mã hợp đồng:</strong> {selectedPlan.tenderNoticeCode}
                        </p>
                        <p>
                            <strong>Tên hợp đồng:</strong> {selectedPlan.packageName}
                        </p>
                        <p>
                            <strong>Danh mục mua sắm:</strong> {selectedPlan.purchaseItems}
                        </p>
                        <p>
                            <strong>Số tiền cần thanh toán:</strong> {selectedPlan.paymentAmount}
                        </p>
                        <p>
                            <strong>Thời gian thanh toán:</strong> {selectedPlan.paymentTime}
                        </p>
                        <p>
                            <strong>Hình thức:</strong> {selectedPlan.paymentMethod}
                        </p>
                    </div>
                    ...
                    <div className="plan-list__batch-actions">
                        <button onClick={handleApproveDetail} className="plan-list__button plan-list__button--approve">
                            Xác nhận thanh toán
                        </button>
                    </div>
                </div>
