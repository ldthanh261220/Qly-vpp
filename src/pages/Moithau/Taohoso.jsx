...
export default function TaoMoiHoSoMoiThau() {
    ...
    useEffect(() => {
        if (selectedGoiThau?.maLinhVuc) {
            axios
                .get(`${process.env.REACT_APP_BACKEND}dsnhathaulv?malinhvuc=${selectedGoiThau.maLinhVuc}`)
    ...
    useEffect(() => {
        axios
            .get(`${process.env.REACT_APP_BACKEND}getdskehoach`)
            .then((res) => {
    ...
    useEffect(() => {
        axios
            .get(`${process.env.REACT_APP_BACKEND}dsgoithau`)
            .then((res) => {
        ...
        <div className="tao-ho-so">
            ...
            {selectedKeHoach && (
                <div className="dual-grid">
                    <div className="grid-item info-box">
                        ...
                        <p>
                            <strong>Tên:</strong> {selectedKeHoach.tenKeHoach}
                        </p>
                        <p>
                            <strong>Mô tả:</strong> {selectedKeHoach.moTa}
                        </p>
                        <p>
                            <strong>Ngày tạo:</strong>{' '}
                            {new Date(selectedKeHoach.thoiGianBatDau).toLocaleDateString('vi-VN')}
                        </p>
                        <p>
                            <strong>Loại yêu cầu:</strong> {selectedKeHoach.loaiyeucau}
                        </p>
                        <p>
                            <strong>Chi phí:</strong> {Number(selectedKeHoach.chiPhiKeHoach).toLocaleString('vi-VN')}VND
                        </p>
                    </div>
                    ...
                    <div className="grid-item product-box">
                        <h4>
                            Danh sách{' '}
                            {selectedKeHoach.loaiyeucau === 'mua sắm' ? 'vật dụng cần mua' : 'thiết bị cần sửa chữa'}
                        </h4>
            ...
            {selectedGoiThau && (
                <div className="info-block mt-3">
                    ...
                    <p>
                        <strong>Tên:</strong> {selectedGoiThau.tenGoiThau}
                    </p>
                    <p>
                        <strong>Mô tả chi tiết:</strong> {selectedGoiThau.moTaChiTiet}
                    </p>
                    <p>
                        <strong>Tên lĩnh vực:</strong> {selectedGoiThau.tenLinhVuc}
                    </p>
